    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:hidden">
        {cameraList.map((camera) => (
          <article key={camera.id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-sm font-medium text-slate-100">{camera.name}</p>
                <p className="mt-0.5 text-xs text-slate-400">{camera.id}</p>
              </div>
              <span
            ...
            <p className="mt-2 text-xs text-slate-300">{camera.location}</p>
          </article>
        ))}
      </div>

      {/* Desktop: table layout */}
      <div className="hidden overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] sm:block">
